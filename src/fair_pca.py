...
class FairPCA:
    ...
    def inverse_transform(self, Z):
        """
        Z (n_test x 2*target_dim numpy-array): data matrix from transform()
        Transforms the data back to the original space.
        """
        W_combined = np.hstack([
            self.transformation_matrix,
            (self.tradeoff_param ** 3) * self.transformation_matrix_standard_PCA
        ])
        X_scaled = Z @ np.linalg.pinv(W_combined)

        if self.standardize:
            X_scaled = self.scaler.inverse_transform(X_scaled)
